...
inline double exc(double);

inline double excp(double);
...
void derivs_Schrodinger(double x, double y[], double dydx[], int k, double T[],\
			double r[], double dr[], int N){
  dydx[1] = y[2]*dr[k];
...
void rk4p480Sch(double y[], double dydx[], int n, double x, double h, double yout[], void (*derivs)(double, double [], double [],int, double [], double [], double [], int), int k, int dk, double f[], double r[], double dr[], int N )
{
  ...
  derivs_Schrodinger(k+h,yt,dyt,(k+dk),f,r,dr,N); //Fourth step.
...
void getphi(double phi[], double Rho[], double r[], double dr[], int N)
{
    ...
    /* Working variables */
    double y[3],dydx[3],*Phi; /* NR vector for diff eq's */
    double x,h;
    int k,dk;
