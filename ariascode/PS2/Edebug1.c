...
main()
{
  ...
  while (1) {
    printf("still alive in Edebug before asking for energy range");
    printf("Search for energy in range (%20.16f,%20.16f):\n",E1,E2);
    printf("N=");
    scanf("%d",&N);
    printf("still alive in Edebug before allocating vectors");
    ...
    printf("still alive in Edebug before initialising r/dr/V");
    ...
    V[0]=0.;
    dr[N]=0.;
    printf("still alive in Edebug before calling rtbisp");
    E=rtbisp480(func_Schrodinger,E1,E2,tol=1e-15, 
                match,V,r,dr,N);
